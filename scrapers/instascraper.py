...
if __name__ == "__main__":
    ...
    try:
        ...
        follower_list=list()
        for count, follower in enumerate(scrape_connections(driver, account=account, connect_type="follower"), 1):
            # print("\t{:>3}: {}".format(count, follower))
            follower_list.append(follower)
            if count >= 1000:
                break
        following_list = list()
        for count, follower in enumerate(scrape_connections(driver, account=account, connect_type="following"), 1):
            # print("\t{:>3}: {}".format(count, follower))
            following_list.append(follower)
            if count >= 1000:s
                break
        
        mutual = intersection(follower_list, following_list)
